...
const cards = [
  ...
  {
    title: "दिवाळी अंक",
    desc: "प्रत्येक वर्षी प्रकाशित होणारा खास दिवाळी अंक म्हणजे नव्या प्रतिभेला वाव देणारा साहित्यिक उत्सव! जगतवाडी आणि जगभरातील मराठी कवींनी लिहिलेल्या दर्जेदार कविता, लघुकथा, लेख आणि अनुभवांचे संकलन इथे वाचायला मिळेल.",
    image: diwali,
    link: "/Diwali"
  },
  {
...
const AamchaSahityaSection = () => {
  const [activeCardIndex, setActiveCardIndex] = useState(0); // 0: ebook, 1: diwali, 2: masik
    ...
    <section className="aamcha-section">
      ...
      {cards.map((card, index) => (
        <div
          ...
          key={index}
          style={{
            display: index === activeCardIndex ? "flex" : "none",
            animation: index === activeCardIndex ? "fadeIn 0.5s ease" : "none"
          }}
        >
          <div className="aamcha-image">
          ...
          <div className="aamcha-card">
            <h3 className="ebook-titlee">{card.title}</h3>
            <p className="ebook-desc">{card.desc}</p>
            <div className="ebook-buttons">
              <Link className="read-button" to={card.link}>वाचा</Link>
              <button className="download-button">Download</button>
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: "10px", marginTop: "2rem" }}>
        {cards.map((_, index) => (
          <button
            key={index}
            className={`read-button ${index === activeCardIndex ? "active" : ""}`}
