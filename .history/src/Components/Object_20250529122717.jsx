...
const AamchaSahityaSection = () => {
  ...
  useEffect(() => {
    const options = {
      threshold: 0.3, // Adjust this value based on when you want the transition to trigger
      rootMargin: "0px 0px -100px 0px" // Adjust this to control when the observer triggers
    };
  ...
  useEffect(() => {
    // Update visibility of all cards based on activeIndex
  ...
  return (
    <section className="aamcha-section">
      ...
      {contentData.map((item, index) => (
        <div
          className={`aamcha-content ${index === 0 ? "visible" : "hidden"}`}
          key={index}
          ref={(el) => (cardRefs.current[index] = el)}
        >
          <div className="aamcha-image">
