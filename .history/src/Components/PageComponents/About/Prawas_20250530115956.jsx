...
const Prawas = () => {
  ...
  const [currentIndex, setCurrentIndex] = useState(2); // Start at 2024
  const dotRefs = useRef([]);
  ...
  useEffect(() => {
    const dotEl = dotRefs.current[currentIndex];
    const yearEl = dotRefs.current[`year-${currentIndex}`];

    if (dotEl?.scrollIntoView) {
      dotEl.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
    }

    if (yearEl?.scrollIntoView) {
  ...
  return (
    <div className='Prawas-Container'>
      ...
      <div className="timeline-container">
        ...
        <div className="timeline-bar">
          ...
          <div className="timeline-years">
            {years.map((year, idx) => (
              <div
                key={idx}
                ref={(el) => (dotRefs.current[`year-${idx}`] = el)}
        ...
        <div className="event-images">
          {currentEvent.images.map((imgSrc, idx) => (
            <img key={idx} src={imgSrc} alt={`event-${idx}`} className="event-image" />
          ))}
        </div>
      </div>
    </div>
